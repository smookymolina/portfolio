...
export default function ProfileCarousel() {
  ...
  return (
    <div
      className="flex flex-col items-center w-full max-w-xs md:max-w-none"
      onMouseEnter={() => setPaused(true)}
      onMouseLeave={() => setPaused(false)}
    >
      {/* Slide Wrapper Container (with fixed dimensions) */}
      ...
      <div className="mt-4 flex flex-col items-center text-center w-full select-none">
        <div className="flex items-center gap-2 w-full mb-1">
          ...
          <span className="font-mono text-xs text-text-muted uppercase">IPN · IAF · México · Italia</span>
          <div className="h-px flex-1 bg-border-subtle" />
        </div>
      </div>
    </div>
